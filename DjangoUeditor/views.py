...
def list_files(request):
    ...
    if (len(files)==0):
        ...
        return_info={
            "state":"SUCCESS",
            "list":files[int(list_start):int(list_start+list_size)],
            "start":list_start,
            "total":len(files)
        }
...
def get_files(root_path,cur_path, allow_types=[]):
    ...
    for item in items:
        ...
        if os.path.isdir(item_fullname):
            ...
            if is_allow_list:
                files.append({
                    "url":os.path.join(USettings.gSettings.MEDIA_URL ,os.path.join(os.path.relpath(cur_path,root_path),item).replace("\\","/" )),
...
def UploadFile(request):
    ...
    # max_size=(request.GET.get(upload_max_size[action],USettings.UEditorUploadSettings.get(upload_max_size[action],0)))
    # if  max_size!=0:
    ...
    #返回数据
    return_info = {
        'url': os.path.join(USettings.gSettings.MEDIA_URL , OutputPathFormat) ,                # 保存后的文件名称
...
def catcher_remote_image(request):
    ...
    max_size=(request.GET.get("catcherMaxSize",USettings.UEditorUploadSettings.get("catcherMaxSize",0)))
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext  in allow_type:
            ...
            catcher_infos.append({
                "state":state,
                "url":os.path.join(USettings.gSettings.MEDIA_URL , o_path_format),
